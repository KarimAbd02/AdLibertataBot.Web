...
namespace AdLibertataBot.Web.Services.Company
{
    public class CompanyAdminService
    {
        ...
        // Добавить этот метод
        public async Task<AdminUser?> GetAdminByCompanyIdAsync(int companyId)
        {
            try
            {
                await using var conn = await _db.CreateConnectionAsync();
                await using var cmd = new NpgsqlCommand(
                    "SELECT id, company_id, username, email, full_name, admin_password, is_active, created_at " +
                    "FROM admin_users WHERE company_id = @company_id AND is_active = true LIMIT 1", 
                    conn);
            ...
            {
                _logger.LogError(ex, "Error getting admin by company ID");
                return null;
            }
        }
    }
